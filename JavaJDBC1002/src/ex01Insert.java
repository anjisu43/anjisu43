...
public class ex01Insert {
	...
	public static void main(String[] args) {
		...
		//연결순서
		//1. oracle에서 제공하는 Java와 연결할 수 있는 외부.jar 가져와야함
		
		//2. ojdbc6.jar에서 연결하는 기능을 담당하는 드라이브.class파일 동적 로딩
		Connection conn = null;
		PreparedStatement psmt = null;
		
		//Java에서 발생하는 오류 2가지
		//1. 컴파일 오류 : 문법적인 오류
		//2. 런타임 오류 : 실행해야지만 알 수 있는 오류(DB, 네트워크)
		//예외처리문 try_catch문
		
		try {//정상실행
			Class.forName("oracle.jdbc.driver.OracleDriver");
			System.out.println("Class 파일 확인");
			//3.Oracle에 가서 DB아이디오 PW를 인증(oracle주소, id, pw)
